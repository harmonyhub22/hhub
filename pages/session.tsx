...
import {Button, Page, Text, Drawer,Modal,useModal,Grid, GeistProvider, CssBaseline, Table } from '@geist-ui/core'
  ...
  return (
    <Page>
      ...
      <Grid.Container gap={1} justify= 'flex-end' height="100px">
        ...
        <Button auto onClick={() => setVisible(true)}>Fisnih song</Button>
        <Modal {...bindings}>
          <Modal.Title>Finishing Song</Modal.Title>
          <Modal.Content>
            <p>Would you like to leave or download the song</p>
          </Modal.Content>
          <Modal.Action passive onClick={() => setVisible(false)}>Leave</Modal.Action>
          <Modal.Action passive onClick={() => saveFile()}>Download</Modal.Action>
        </Modal>
        <input
        ...
        />
        <Button auto onClick={() => setState(true)} scale={1}>Show Pallete</Button>
